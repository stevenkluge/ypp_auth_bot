...
export const fetchPirateCrewRank = async (
  pirate: string,
): Promise<{
  ...
  try {
    const $ = cheerio.load(
      await fetchDocument(
        `http://emerald.puzzlepirates.com/yoweb/pirate.wm?target=${pirate}`,
      ),
    );
    const crewInfoElements = $(
      "body > table:nth-child(1) > tbody:nth-child(1) > tr:nth-child(1) > td:nth-child(1) > table:nth-child(1) > tbody:nth-child(1) > tr:nth-child(2) > td:nth-child(1) > table:nth-child(1) > tbody:nth-child(1) > tr:nth-child(1) > td:nth-child(2) > font:nth-child(1)",
    );
    const rank = crewInfoElements
      ?.text()
      ?.match(/^(?: ?\w+){1,2}(?= (?:and .*)?of the crew)/)?.[0];
    const crew = crewInfoElements
      ?.find("a")
      ?.attr("href")
      ?.match(/(?<=crewid=)\d{7}/)?.[0];
    ...
    if (!!crew && !!rank) {
      ...
      $("body > center:nth-child(1)").text().trim() ===
      "Arr! We have heard no tell of that pirate around these parts."
    ) {
      ...
      logger.warn(
        `Pirate either doesn't have a crew, or there is something wrong with the scraper | query: ${pirate}`,
      );
      return {
    ...
    logger.error(
      `Error while attempting to retrieve pirate page | query: ${pirate} | ${err}`,
    );
    return null;
  }
};

export const fetchTrophyCollectionBoxNames = async (
  pirate: string,
): Promise<Array<string> | null> => {
  try {
    const $ = cheerio.load(
      await fetchDocument(
        `https://emerald.puzzlepirates.com/yoweb/trophy/?pirate=${pirate}`,
      ),
    );
    const trophyCollectionBoxes = $(
      "body > center:nth-child(1) > table:nth-child(2n+6) > tbody:nth-child(1) > tr:nth-child(2) > td:nth-child(2) > center:nth-child(1) > font:nth-child(1) > b:nth-child(1)",
    );
    const trophyCollectionStrings = trophyCollectionBoxes
      .map((idx, element) => $(element).text())
      .toArray();
    ...
    if (trophyCollectionStrings?.length > 0) {
      ...
      logger.warn(
        `Could not find trophy collection box titles | query: ${pirate}`,
      );
      return null;
    }
  } catch (err) {
    logger.error(
      `Error while attempting to retrieve trophy page | query: ${pirate} | ${err}`,
    );
    return null;
  }
};

export const fetchCrewRoster = async (
  crew: string,
): Promise<RosterType | null> => {
  try {
    const $active = cheerio.load(
      await fetchDocument(
        `https://emerald.puzzlepirates.com/yoweb/crew/info.wm?crewid=${crew}`,
      ),
    );
    const $dormant = cheerio.load(
      await fetchDocument(
        `https://emerald.puzzlepirates.com/yoweb/crew/dormant_members.wm?crewid=${crew}`,
      ),
    );
    ...
    const activeRoster = $active(
      "body > table:nth-child(1) > tbody:nth-child(1) > tr:nth-child(3) > td:nth-child(3) > table:nth-child(1) > tbody:nth-child(1) > tr:nth-child(2) > td:nth-child(3) > table:nth-child(1)",
    )
      .text()
    ...
    logger.error(
      `Error while attempting to retrieve crew page | query: ${crew} | ${err}`,
    );
  }
};
...
const fetchDocument = async (resourceUrl: string) => {
  ...
  if (!response.ok)
    throw new Error(
      `fetch(${resourceUrl}) returned ${response.status}: ${response.statusText}`,
    );
